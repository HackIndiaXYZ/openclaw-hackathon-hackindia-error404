...
export function useNexus() {
  ...
  const [globalRank, setGlobalRank] = useState<{ rank: string | number, tier: KarmaTier }>({ rank: '---', tier: 'bronze' });
  ...
  const fetchCrossCampusProfile = async (uid: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setGlobalRank(data.data.globalRank || { rank: '---', tier: 'bronze' });
      }
    } catch (error: any) {
  ...
  const adminOverride = async (swapId: string, action: 'force_refund' | 'force_payout', notes: string) => {
    try {
      await apiClient.post(`/admin/swaps/${swapId}/override`, { action, notes });
      toast.success(`Override Executed: ${action.replace('_', ' ')}`);
      fetchSwaps();
    } catch (error: any) {
