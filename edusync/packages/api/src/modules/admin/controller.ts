...
/**
 * Legacy mocked moderation queue - deprecated in favor of GuardianService.
 */
...
/**
 * Legacy resolve function - now using VerificationService for resources.
 */
export const resolveFlag = async (req: Request, res: Response) => {
  try {
    const { action, resourceId, adminNotes } = req.body;
    const adminUid = (req as any).student.uid;
    const adminCampus = (req as any).student.campus;
    
    if (resourceId) {
      if (action === 'approve') {
        await VerificationService.approveResource(resourceId, adminUid, adminCampus, adminNotes);
      } else if (action === 'reject') {
        await VerificationService.rejectResource(resourceId, adminUid, adminCampus, adminNotes, 'other');
      }
    }

    res.json({ message: 'Guardian Protocol: Resolution Complete', timestamp: new Date() });
  } catch (err: any) {
